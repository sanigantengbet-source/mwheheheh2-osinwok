...
import { 
  ...
  BookOpen, HelpCircle, FileText, Star
} from "lucide-react";
import * as motion from "motion/react-client";

const testimonials = [
  { name: "Budi S.", date: "12/07/2026", title: "Sangat Cepat & Akurat", text: "Sangat membantu untuk verifikasi data dengan cepat. Tidak perlu instalasi, langsung jadi di browser. Terbaik pokoknya!", rating: 5 },
  { name: "Andi M.", date: "10/07/2026", title: "UI Bersih & Modern", text: "UI-nya sangat bersih dan modern, prosesnya instan! Sangat berguna untuk mengecek validitas NIK saat pendaftaran.", rating: 5 },
  { name: "Siti N.", date: "05/07/2026", title: "Alat OSINT Andalan", text: "Alat OSINT terbaik untuk NIK sejauh ini, privasi terjaga dan tidak menyimpan history pencarian kita sama sekali.", rating: 4 },
  { name: "Rina A.", date: "28/06/2026", title: "Mudah Digunakan", text: "Gampang digunakan, hasil akurat dan detail. Bahkan breakdown lokasinya sangat pas dengan KTP asli.", rating: 5 },
  { name: "Dimas K.", date: "15/06/2026", title: "Sangat Berguna", text: "Luar biasa, sangat berguna untuk pekerjaan lapangan yang butuh validasi identitas secara real-time.", rating: 5 },
];

const techStacks = [
  { name: "Next.js", icon: <svg viewBox="0 0 128 128" className="w-6 h-6"><path fill="currentColor" d="M64 0C28.7 0 0 28.7 0 64s28.7 64 64 64c11.3 0 21.9-2.9 31.2-8l-40.4-53.7-18.4 24.5-9.3-12.4 27.7-36.9 49.3 65.5C118.4 93.6 128 79.9 128 64c0-35.3-28.7-64-64-64zm31.8 101.4L51.3 42.1h10.9l38.2 50.8c-1.5 1.4-3.1 2.8-4.6 4.1v4.4zm10.7-59.3h-10v45.1c3.8-5.3 6.9-11.2 9-17.5V42.1h1z"/></svg> },
  { name: "React", icon: <svg viewBox="-11.5 -10.23174 23 20.46348" className="w-6 h-6 text-[#61DAFB]"><circle cx="0" cy="0" r="2.05" fill="currentColor"/><g stroke="currentColor" strokeWidth="1" fill="none"><ellipse rx="11" ry="4.2"/><ellipse rx="11" ry="4.2" transform="rotate(60)"/><ellipse rx="11" ry="4.2" transform="rotate(120)"/></g></svg> },
  { name: "Tailwind CSS", icon: <svg viewBox="0 0 24 24" className="w-6 h-6 text-[#06B6D4]"><path fill="currentColor" d="M12 5.5c-3 0-5 2.5-5 6.5 2-2 4-2.5 5.5-1.5 1 1 1.5 2.5 1.5 4.5s-2 3.5-5 3.5c-3 0-5-2.5-5-6.5s2.5-7 6.5-7c2 0 4 1 5.5 2.5 1 1.5 1.5 3 1.5 4.5s-2 3.5-5 3.5c-3 0-5-2.5-5-6.5 0-3 2-4.5 4-5.5.5-.5 1-1 1.5-1z"/></svg> },
  { name: "JavaScript", icon: <svg viewBox="0 0 24 24" className="w-6 h-6 text-[#F7DF1E]"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M22.034 18.276c-.175-1.095-.888-2.015-3.003-2.873-.736-.345-1.554-.585-1.797-1.14-.091-.33-.105-.51-.046-.705.15-.463.815-.9 2.155-.84l.034.004c1.233.027 1.838.647 2.052.923l2.673-1.637c-.302-.379-1.284-1.516-3.238-1.921-1.019-.208-2.293-.167-3.486.34-1.258.533-2.115 1.402-2.5 2.457-.354.966-.35 2.018.156 2.925.541.97 1.498 1.503 2.99 2.115 1.15.474 1.706.845 1.874 1.341.058.172.072.483-.092.813-.23.473-.836.936-2.31.89-1.393-.042-2.348-.79-2.656-1.066l-2.68 1.62c.21.282 1.394 1.696 3.553 2.213 1.15.275 2.51.24 3.793-.3 1.385-.584 2.227-1.442 2.628-2.576.32-.906.32-2.003-.09-2.883zm-11.455.578c.032.54.343.832.617 1.011 1.094.717 2.45 1.077 4.025.753v-2.306c-.663.262-1.396.402-2.154.402-.555 0-1.104-.08-1.634-.236-.37-.11-.534-.232-.612-.348-.124-.187-.21-.5-.21-1.011V6.993H7.07v9.426c0 1.25.32 2.181.933 2.723.633.562 1.502.834 2.583.834z" fill="currentColor"/></svg> },
  { name: "TypeScript", icon: <svg viewBox="0 0 24 24" className="w-6 h-6 text-[#3178C6]"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M22.034 18.276c-.175-1.095-.888-2.015-3.003-2.873-.736-.345-1.554-.585-1.797-1.14-.091-.33-.105-.51-.046-.705.15-.463.815-.9 2.155-.84l.034.004c1.233.027 1.838.647 2.052.923l2.673-1.637c-.302-.379-1.284-1.516-3.238-1.921-1.019-.208-2.293-.167-3.486.34-1.258.533-2.115 1.402-2.5 2.457-.354.966-.35 2.018.156 2.925.541.97 1.498 1.503 2.99 2.115 1.15.474 1.706.845 1.874 1.341.058.172.072.483-.092.813-.23.473-.836.936-2.31.89-1.393-.042-2.348-.79-2.656-1.066l-2.68 1.62c.21.282 1.394 1.696 3.553 2.213 1.15.275 2.51.24 3.793-.3 1.385-.584 2.227-1.442 2.628-2.576.32-.906.32-2.003-.09-2.883zM10.835 11.23V6.992H.994v4.238h4.093v12.77h4.225v-12.77h1.523z" fill="currentColor"/></svg> },
  { name: "Vercel", icon: <svg viewBox="0 0 24 24" className="w-6 h-6 text-white"><path fill="currentColor" d="M12 1L24 22H0L12 1z"/></svg> },
];

const doubledTestimonials = [...testimonials, ...testimonials];
const doubledTechStacks = [...techStacks, ...techStacks, ...techStacks, ...techStacks];
    ...
    <div className="min-h-screen flex flex-col font-sans selection:bg-white/20">
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-6 py-10 md:py-16">
        ...
        <section className="mb-8 overflow-hidden">
          ...
          <div className="flex gap-4 group mx-auto max-w-full" style={{ maskImage: "linear-gradient(to right, transparent, black 10%, black 90%, transparent)" }}>
            <div className="flex shrink-0 gap-4 animate-marquee-left group-hover:pause">
              {doubledTestimonials.map((item, i) => (
                <div key={i} className="w-[300px] sm:w-[350px] p-5 bg-[#1a1a1a] border border-white/5 rounded-xl flex-shrink-0 hover:bg-[#222] transition-colors">
                  ...
                  <div className="flex gap-1 mb-3">
                    {Array.from({ length: 5 }).map((_, j) => (
                      <Star key={j} className={`w-3 h-3 ${j < item.rating ? 'text-amber-500 fill-amber-500' : 'text-gray-600'}`} />
                    ))}
                  </div>
            ...
            <div className="flex shrink-0 gap-4 animate-marquee-left group-hover:pause" aria-hidden="true">
              {doubledTestimonials.map((item, i) => (
                <div key={`dup-${i}`} className="w-[300px] sm:w-[350px] p-5 bg-[#1a1a1a] border border-white/5 rounded-xl flex-shrink-0 hover:bg-[#222] transition-colors">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="text-sm font-semibold text-white">{item.title}</h3>
                    <div className="text-right">
                      <span className="text-[10px] text-gray-500 block">{item.date}</span>
        ...
        <section className="mb-20 overflow-hidden">
          <div className="flex gap-4 group mx-auto max-w-full" style={{ maskImage: "linear-gradient(to right, transparent, black 10%, black 90%, transparent)" }}>
            <div className="flex shrink-0 gap-8 items-center animate-marquee-right group-hover:pause">
              {doubledTechStacks.map((item, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-2 bg-white/5 border border-white/10 rounded-full flex-shrink-0">
                  {item.icon}
                  <span className="text-xs font-semibold text-gray-300 tracking-wider">{item.name}</span>
                </div>
              ))}
            </div>
            <div className="flex shrink-0 gap-8 items-center animate-marquee-right group-hover:pause" aria-hidden="true">
              {doubledTechStacks.map((item, i) => (
                <div key={`dup-${i}`} className="flex items-center gap-3 px-4 py-2 bg-white/5 border border-white/10 rounded-full flex-shrink-0">
                  {item.icon}
                  <span className="text-xs font-semibold text-gray-300 tracking-wider">{item.name}</span>
                </div>
              ))}
